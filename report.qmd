...
Duplicate rows were removed by retaining the last term row per student (*n* = ). Students whose last term row did not match their graduation code were removed (*n* = ), as time series variables were not included in their graduation term. Additionally, current students (n = 2,053), students who declared two or more majors at any term (*n* = ), and those who were in a Master's program were removed (*n* =). After these exclusions, N = term rows and N = students remained.

The Did_Graduate column was re-coded to represent students who: Did not graduate (0), graduated with one of the majors within the computer science program (), or graduated with another major (). Finally, rows corresponding to terms outside of a student's enrollment in computer science program were excluded (n = ), leaving the final dataset for
pivoting (N = ).

### Data Transformation

The dataset was pivoted to to represent one row per student to ensure compatibility with statistical methods and reduce redundancy.
