...
by white males (citation and numbers needed). Limited minority representation within the profession
...
rights and democratic values (citation needed). Understanding the factors contributing to
...
(TODO: insert lit review of previous, similar work)
...
and analyzing the data. The @sec-data section [characterizes the original dataset](@sec-data_descr), and then explains how and why the original dataset was [cleaned](@sec-data_clean),
[pivoted](@sec-data_pivot), and [subset](@sec-data_subset). The @sec-descr_stats section graphs key
metrics of the [first](@sec-descr_1) and [second](@sec-descr_2) samples taken from the dataset. Finally, the @sec-stats_back
section details the theoretical foundations and assumptions of [chi-square](@sec-cs_back), [logistic regression](@sec-lr_back), and [survival analysis](@sec-sa_back) — the three statistical methods used
...
conduct this research. Her continued support was much appreciated throughout the entire process.
