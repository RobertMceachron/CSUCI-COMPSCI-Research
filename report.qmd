...
log <- sdata %>%
  select(Is_Hispanic, Gender_Binary, Housing_All,
         Did_Graduate, AverageUnits, Tutoring_All)

#AverageUnits Binning
sdata$AverageUnits_bin <- cut(log$AverageUnits,
                                breaks = c(0, 11, 15, 18),
                                include.lowest = TRUE,
                                labels = c("Parttime", "Fulltime", "Heavy"))

#Part-time: 6-11, Medium(Full Time): 12-15, Heavy: 16-17
table(sdata$AverageUnits_bin)# Check counts
#High is super low :/

#Medium as reference
sdata$AverageUnits_bin <- factor(sdata$AverageUnits_bin,
                                   levels = c("Fulltime", "Parttime", "Heavy"))

#Tutoring_All binning
sdata$Tutoring_bin <- cut(sdata$Tutoring_All,
...
\[number\]) graduated with a Bachelor's degree from the Department of
