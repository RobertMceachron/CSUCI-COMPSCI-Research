...
############################################
# ADDITIONAL CLEANING FROM MONI'S ANALYSIS #
############################################

# We're using the "pivoted" dataframe as our starting point
...
#ddata$Did_Graduate_All <- BinaryChecker(ddata[, grep("_Graduate$", names(ddata))]) # got rid of this bc it's not necessary for multinomial regression
...
#   -- Did_Graduate fits this!
# - The relative likelihood of the response variable being in one category compared to the base category would not change with the addition of more categories (Independence of Irrelevant Alternatives)
#   -- This one is tricky.  I'd feel better about satisfying this assumption if we had information about what goes into the "did not graduate" category, like "transfer to another school" or "drop out of college"
# - The observations are independent
#   -- Pivoted data here
# - There exist linear relationships between the explanatory variables and the relative log-odds of the response variable
#   -- Maybe?  Unclear
# - There are no extreme outliers that pull any of the logistic regression lines
#   -- There are some pretty extreme outliers... jury's still out on how badly they pull the regression line though
# - The explanatory variables are not correlated with each other
#   -- Considering we pivoted the time series so that the time intervals are next to each other horizontally, I don't think that this assumption is met
#   -- Assuming, of course, we use the individual time series variables instead of averages
#   -- If we use average or medians, it's probably fine
#      --- I'm using averages in the dataset for the multinomial regression
# - The sample size is sufficiently large
#   -- Sure??? idk what would count as "sufficiently large" for us lol

###########################################
# THE ACTUAL MULTINOMIAL REGRESSION MODEL #
###########################################

mnom_data <- ddata %>% select(Student, Is_Hispanic, Gender_Binary, Is_Transfer, Dependent_Income_Code, Housing_All, Tutoring_All, Units_Avg, Terms_Attended, Did_Graduate)
grad_multinom <- multinom(formula = Did_Graduate ~ Is_Hispanic + Gender_Binary + (Is_Hispanic * Gender_Binary) + Is_Transfer + Dependent_Income_Code + Housing_All + Tutoring_All + Units_Avg + Terms_Attended, data = mnom_data)
results <- tidy(grad_multinom, conf.int = TRUE)
results